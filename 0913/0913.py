...
def handle_bar(context): 
    ...
    k1,k2=0.1,0.1
    fifmin_open  =   history_bars(context.s1, 7, '15min', 'open',True)
    fifmin_close =   history_bars(context.s1, 7, '15min', 'close',True)
    fifmin_high  =   history_bars(context.s1, 7, '15min', 'high',True)
    fifmin_low   =   history_bars(context.s1, 7, '15min', 'low',True)

    if len(fifmin_close)<=7:
        return
    HH=fifmin_high[-2:-7].max()
    HC=fifmin_close[-2:-7].max()
    LC=fifmin_close[-2:-7].min()
    LL=fifmin_low[-2:-7].min()
    range=max(HH-LC,HC-LL)
    buy_line=fifmin_open[-2:-7]+range*k1
    sell_line=fifmin_open[-2:-7]+range*k2

    if context.long_condition and (fifmin_high[-1]>buy_line[-1]):
        DT_con=True
    if context.short_condition and (fifmin_low[-1]<sell_line[-1]):
        DT_con=True


    if context.on_the_window:
        if context.long_condition and context.open_condition and DT_con:
            buy_open(context.s1, "market", volume=1,serial_id = 1)
            context.long_condition=False
        elif context.short_condition and context.open_conditionand DT_con:
